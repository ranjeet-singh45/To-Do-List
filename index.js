...
app.post("/edit", async (req, res) => {
  ...
  try {
    await db.query("UPDATE items1 SET title = ($1) WHERE id = $2", [item1, id1]);
    await db.query("UPDATE items2 SET title = ($1) WHERE id = $2", [item2, id2]);
    await db.query("UPDATE items3 SET title = ($1) WHERE id = $2", [item3, id3]);
    res.redirect("/");
  } catch (err) {
    console.log(err);
  }
